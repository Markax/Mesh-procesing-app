/* Este código tiene que ser compilado desde el prompt de MATLAB con el siguiente comando: 
     - mex compute_local_shfd.cpp surface_processing.cpp -lmwlapack -lmwblas -largeArrayDims -v 
...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, mxArray *prhs[] )
{
  ...
  if(nrhs!=3) 
    mexErrMsgTxt("Se requieren 3 parámetros: fichero con áreas suavizadas, L mínima y L máxima para ajuste de recta de regresión");
    
  if ( mxIsChar(prhs[0]) != 1 )
    mexErrMsgTxt("Primer parámetro debe ser el nombre del fichero con las áreas suavizadas");
  ...
  InName1 = mxArrayToString(prhs[0]);
	...
	sup_global.calcula_Local_SH_FD_por_vertices((std::string) InName1, (int)*InName2, (int)*InName3);
}
