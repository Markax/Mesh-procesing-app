function [currentDir] = BATCH_MLCombineAndResize(currentDir, x, y, z)
% Modificado por Juan para poder ser llamado en modo BATCH
% la entrada debe ser un fichero .m tal y como lo genera uja-3dshfd
% independiente de los parámetros x, y y z que se establezcan, hace siempre
% un centrado en el origen sin resize y sin hacer salida output
...
wb = waitbar(0, 'Repositionating in the origin...  (2/6)', 'Name', 'Generating Spherical Harmonics');
...
waitbar(1/n+4, wb, 'Repositionating in the origin...  (2/6)');
...
waitbar(2/n+4, wb, 'Repositionating in the origin...  (2/6)');
% Process files
for i = 1:n;
    
    waitbar(2+i/n+4, wb, 'Repositionating in the origin...  (2/6)');
    ...
    elseif (strcmpi(ext, '.stl'))
        ...
        name = strtok(name,'.'); % para quitar si hubiera otra extension ademas de .stl
    end
...
%disp('Resizing by Distances.');
    ...
    waitbar(3+n/n+4, wb, 'Repositionating in the origin...  (2/6)');
...
waitbar(1, wb, 'Repositionating in the origin...  (2/6)'); 
