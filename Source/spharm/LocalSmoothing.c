...
#define LWSIZE 10000
#define SVSIZE 10000
mwSize LWORK = LWSIZE; /* workspace dimension  */
...
void local_smooth(char *InName, char *OutName)
{
	...
	map = get_locsmo_values(InName,&reso,&extents);
        ...
        strcpy(fichero,InName);
        strcat(fichero,"_metric");
        put_vector_values(/*"metric"*/fichero,metric,METRICNUM);  /* put metric value into file */
...
unsigned relocate(double *newvs, double *verts, int ix, MAPPING *map)
{
	...
	for(i=0;i<nbnum;i++){
        ...
        /* printf("--- Entra extract_locmesh_verts ---\n"); */
	lvs = extract_locmesh_verts(verts,vnum,ix,nbvs,nbnum);
        /* printf("--- Sale extract_locmesh_verts ---\n"); */

        /* printf("--- Entra lsq_area_posi ---\n"); */
	flatvs = lsq_area_posi(newposi,lvs,map,stda); /* solve using least square  */
        /* printf("--- Sale lsq_area_posi ---\n"); */
...
void create_nbs(MAPPING *map)
{
	...
	/* printf("pasa el segundo for\n"); */
    
	/* move to mapnbs  */
	move_elists_to_mapnbs(map,elists);
	/* show_nbs(map->nbs, map->num[1]);  */

        /* printf("pasa el move_elists_to_mapnbs\n"); */
...
void move_elists_to_mapnbs(MAPPING *map, EDGELIST **elists)
{
	...
	/* printf("entra en move_elists_to_mapnbs\n"); */
...
void solve_dgels(LEQS *leqs)
{
	...
	lwork = max(max(m,n),NRHS)*8+min(m,n);
        ...
        /* printf("m=%d, n=%d, A[%d,%d]=%f, b[%d]=%d\n", m,n,m-1,n-1,A[m-1,n-1],m-1,b[m-1]); */
        /* LWORK = 0; */
	if (lwork<=LWORK){
                /* printf("--- Entra dgels de IF ---\n");*/
		BLASCALL(dgels)("N", &m, &n, &NRHS, A, &m, b, &m, WORK, &LWORK, &ok);
                /* printf("ok=%d\n",ok); */ 
                /* printf("--- Sale dgels de IF ---\n"); */      
	}
	else{
		printf(">>> New lwork=%d\n",lwork);
		...
		integer *, integer *); */
                /* printf("--- Entra dgels de ELSE ---\n"); */
		BLASCALL(dgels)("N", &m, &n, &NRHS, A, &m, b, &m, work, &lwork, &ok);
                /* printf("--- Sale dgels de ELSE ---\n"); */      
		free(work);
	}
